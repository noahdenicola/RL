...
class Network(nn.Module):
    def __init__(self, hidden, dropout):
        ...
        #Layers
        self.fc1 = nn.Linear(4, hidden[0])
        self.fc2 = nn.Linear(hidden[0],2)
...
class DQN_Agent():
    def __init__(self, hidden, lr, discount, K, dropout, lossFunc):
        ...
        self.target = self.main#Network(hidden, 0.0)
        self.target.dropout = nn.Dropout(0.0)

        #Hyper parameters
        self.lr = lr
        self.discount = discount
        self.A = torch.rand([K, 4])
...
def auto_train(config=None):
    ...
    with wandb.init(config=config):
        ...
        while steps < config.totalSteps:
            ...
            #Evaluate Agent
            if not solved:
                if e%100 == 0 or score==500: #Every 100 epochs or when a perfect score is reached
                    total = 0
                    for p in range(100):
            ...
            if aveScore >= 500 and not solved:
                print("")
...
def main():
    ...
    parameters_dict = { "env":          {'value' : "CartPole-v1"},
                        "playLength":   {'value' : 500 }, 
                        "lossFunc":     {'value' : 'Huber'},
                        'lr' :          {'value' : 0.005 },
                        'K':            {'value' : 64 },
                        'update_every': {'value' : 3},
                        'C' :           {'value' : 4},
                        "totalSteps":   {'value' : 20000}, 
    ...
    sweep_id = wandb.sweep(sweep_config, project="CartPole_CMS-DQN")
